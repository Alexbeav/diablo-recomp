...
#
# Writes: dist/diablo-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix diablo-recomp \
  --exe-name Diablo_Recompiled \
  --display-name "Diablo Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Diablo disc" \
